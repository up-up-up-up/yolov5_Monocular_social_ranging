...
def distancing_1(person_coords, car_coords,dis_m1,dis_m, sq, img):
    already_red = dict()
    centers_people = []
    centers_car = []
    centers = []
    for i in person_coords:
    ...
    for j in centers:
        already_red[j] = 2
    x_combs = itertools.product(person_coords, car_coords, repeat=1)
    radius = 10
    thickness = 5
    for x in x_combs:
        xyxy1, xyxy2 = x[0],x[1]
...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            im = distancing_1(person_coords, car_coords, dis_m1, dis_m, sq, im0)
            '''if view_img:
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default=ROOT / 'data/images/3.mp4', help='file/dir/URL/glob, 0 for webcam')
